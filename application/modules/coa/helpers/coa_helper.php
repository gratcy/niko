...
function __extract_coa(array $arr, $pass=0) {
	...
	foreach($arr as $k => $v) {
		...
		$res .= '<td>';
		$res .= '<a href="'.site_url('coa/coa_update/' . $v -> cid).'"><i class="icon-pencil"></i></a> <a href="'.site_url('coa/coa_delete/' . $v -> cid).'" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="icon-remove"></i></a>';
		$res .= '</td>';
		$res .= '</tr>';
