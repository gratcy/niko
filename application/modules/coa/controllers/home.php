...
class Home extends MY_Controller {
	...
	function index() {
		$pager = $this -> pagination_lib -> pagination($this -> coa_model -> __get_coa($this -> memcachedlib -> sesresult['ubid']),3,150,site_url('coa'));
		...
		if ($_POST) {
			...
			//~ }
				...
				if ($this -> coa_model -> __insert_coa($arr, 1)) {
					$arr2 = array('cbid' => $this -> memcachedlib -> sesresult['ubid'], 'cidid' => $this -> db-> insert_id(), 'csaldo' => $saldo);
	...
	function coa_update($id) {
		if ($_POST) {
			...
			if ($id) {
				...
				//~ }
					...
					if ($this -> coa_model -> __update_coa($id, $arr, 0, 1)) {	
						if ($this -> coa_model -> __check_coa_detail($id, $this -> memcachedlib -> sesresult['ubid']) > 0) {
							$arr2 = array('csaldo' => $saldo);
							$this -> coa_model -> __update_coa($id, $arr2, $this -> memcachedlib -> sesresult['ubid'], 2);
						}
						else {
							$arr2 = array('cbid' => $this -> memcachedlib -> sesresult['ubid'], 'cidid' => $id, 'csaldo' => $saldo);
							$this -> coa_model -> __insert_coa($arr2, 2);
						}
			...
			$view['id'] = $id;
			$view['detail'] = $this -> coa_model -> __get_coa_detail($id, $this -> memcachedlib -> sesresult['ubid']);
