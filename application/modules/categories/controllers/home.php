...
class Home extends MY_Controller {
	...
	function categories_add() {
		if ($_POST) {
			...
			$discearly = (float) $this -> input -> post('discearly');
			$disctrans = (float) $this -> input -> post('disctrans');
			...
			if (!$name || !$desc) {
				...
				$arr = array('cname' => $name, 'cdesc' => $desc, 'cdiscount' => $disc, 'cdiscounttrans' => $disctrans, 'cdiscountdate' => $discearly, 'ctype' => 1, 'cstatus' => $status);
