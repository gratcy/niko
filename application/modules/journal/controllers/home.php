...
class Home extends MY_Controller {
	...
	function index() {
		$pager = $this -> pagination_lib -> pagination($this -> journal_model -> __get_journal($this -> memcachedlib -> sesresult['ubid']),3,10,site_url('journal'));
	...
	function journal_add() {
		if ($_POST) {
			...
			elseif ($numrows == 0) {
				...
				$arr = array('gbid' => $this -> memcachedlib -> sesresult['ubid'], 'guid' => $this -> memcachedlib -> sesresult['uid'], 'gaid' => $aid[0] -> aid, 'gtype' => $type, 'gtitle' => $title, 'gdesc' => $desc, 'gdate' => time(), 'gdocref' => $docref, 'gstatus' => $status);
